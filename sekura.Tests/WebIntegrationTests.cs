...
internal static class TestSqliteDatabase
{
    ...
    public static DbContextOptions<SqliteSekuraDbContext> CreateOptions(string databasePath)
    {
        EnsureDatabaseFile(databasePath);

        var builder = new DbContextOptionsBuilder<SqliteSekuraDbContext>();
        builder.UseSqlite(
            CreateConnectionString(databasePath),
            sqlite => sqlite.MigrationsAssembly(typeof(SqliteSekuraDbContext).Assembly.FullName));
        return builder.Options;
    }
}

internal sealed class TestSqliteSekuraDbContextFactory : ISekuraDbContextFactory
{
    private readonly DbContextOptions<SqliteSekuraDbContext> _options;

    public TestSqliteSekuraDbContextFactory(DbContextOptions<SqliteSekuraDbContext> options)
    {
        _options = options;
    }
    ...
    public Task<SekuraDbContext> CreateDbContextAsync(CancellationToken cancellationToken = default)
    {
        ...
        return Task.FromResult<SekuraDbContext>(new SqliteSekuraDbContext(_options));
    }
}
...
public class WebIntegrationTests : IClassFixture<TestWebApplicationFactory>
{
    ...
    public async Task LocalUser_WithRequiredTotp_CompletesAuthenticatorOnboardingBeforeSignIn()
    {
        ...
        Assert.Contains("Secure shares", dashboardHtml, StringComparison.OrdinalIgnoreCase);
    ...
    {
        await using var factory = new SqliteTestWebApplicationFactory();
        var dbContextFactory = factory.Services.GetRequiredService<ISekuraDbContextFactory>();
        var localUserService = factory.Services.GetRequiredService<ILocalUserService>();
        var passwordCryptoService = factory.Services.GetRequiredService<IPasswordCryptoService>();
    ...
    public async Task ShareAccess_FailedAttemptsPauseOnlyThatShare_UsingConfiguredSettings()
    {
        ...
        var recipientEmail = $"recipient-{Guid.NewGuid():N}@example.com";
        var created = await CreateShareAsync(client, recipientEmail, "locked.share", "LockedSekura!123");
        ...
        Assert.DoesNotContain("LockedSekura!123", blockedSuccessHtml, StringComparison.Ordinal);
...
{
    private readonly string _databaseDirectory = Path.Combine(Path.GetTempPath(), "sekura-tests", Guid.NewGuid().ToString("N"));
    ...
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        ...
        {
            services.RemoveAll<DbContextOptions<SqliteSekuraDbContext>>();
            services.RemoveAll<ISekuraDbContextFactory>();
            ...
            services.AddSingleton<ISekuraDbContextFactory, TestSqliteSekuraDbContextFactory>();
...
internal sealed class SqliteTestWebApplicationFactory : WebApplicationFactory<Program>
{
    ...
    public string DatabasePath => Path.Combine(_databaseDirectory, "sekura.sqlite");
...
internal sealed class TestSqlitePlatformInitializationService : IPlatformInitializationService
{
    ...
    public TestSqlitePlatformInitializationService(
        ISekuraDbContextFactory dbContextFactory,
...
{
    private readonly ConcurrentDictionary<Guid, SecureShare> _shares = new();

    public Task<IReadOnlyCollection<SecureShare>> GetAllSharesAsync(CancellationToken cancellationToken = default)
    {
        ...
        return Task.FromResult<IReadOnlyCollection<SecureShare>>(list);
    }

    public Task<SecureShare?> GetShareByIdAsync(Guid id, CancellationToken cancellationToken = default)
    {
        _shares.TryGetValue(id, out var share);
        return Task.FromResult(share is null ? null : Clone(share));
    }

    public Task<SecureShare?> GetShareByTokenAsync(string token, CancellationToken cancellationToken = default)
    {
        var share = _shares.Values.FirstOrDefault(x => x.AccessToken == token);
        return Task.FromResult(share is null ? null : Clone(share));
    }

    public Task UpsertShareAsync(SecureShare share, CancellationToken cancellationToken = default)
    {
    ...
    private static SecureShare Clone(SecureShare share)
    {
