...
public class KeyVaultStore : IShareStore, IInformationRequestStore
{
    ...
    public KeyVaultStore(SecretClient secretClient, IOptions<AzureKeyVaultOptions> options)
    {
        ...
        _prefix = string.IsNullOrWhiteSpace(options.Value.SecretPrefix) ? "sekura" : options.Value.SecretPrefix.Trim().ToLowerInvariant();
    }

    public async Task<IReadOnlyCollection<SecureShare>> GetAllSharesAsync(CancellationToken cancellationToken = default)
    {
        var shares = new List<SecureShare>();
    ...
    private async Task<SecureShare?> TryGetShareFromSecretNameAsync(string secretName, CancellationToken cancellationToken)
    {
        try
        {
            var response = await _secretClient.GetSecretAsync(secretName, cancellationToken: cancellationToken);
            return JsonSerializer.Deserialize<SecureShare>(response.Value.Value, JsonOptions);
        }
