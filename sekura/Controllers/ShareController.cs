...
public class ShareController : Controller
{
    ...
    public async Task<IActionResult> Access(string token)
    {
        ...
        if (share?.RequireOidcLogin == true)
        {
            ...
            {
                await _auditLogger.LogAsync("oidc-user", oidcEmail, "share.access", false, "SecureShare", share.Id.ToString(), "OIDC recipient mismatch.");
                return Forbid();
            }
    ...
    public async Task<IActionResult> Access(ShareAccessViewModel model)
    {
        ...
        if (share?.RequireOidcLogin == true)
        {
            ...
            {
                await _auditLogger.LogAsync("oidc-user", email, "share.access", false, "SecureShare", share.Id.ToString(), "OIDC recipient mismatch.");
                return Forbid();
            }
        }
        ...
        if (share.ExpiresAtUtc <= _applicationTime.UtcNow)
        {
            ...
            await _auditLogger.LogAsync("external-user", email, "share.access", false, "SecureShare", share.Id.ToString(), "Share expired.");
            ModelState.AddModelError(string.Empty, "This secure share has expired.");
            return View(model);
        }
        ...
        {
            await _auditLogger.LogAsync("external-user", email, "share.access", false, "SecureShare", share.Id.ToString(), exception.DiagnosticMessage);
            ModelState.AddModelError(string.Empty, exception.UserMessage);
            return View(model);
        }

        await _auditLogger.LogAsync("external-user", email, "share.access", true, "SecureShare", share.Id.ToString());
        ...
        {
            await _auditLogger.LogAsync("system", "mail-service", "mail.share-access.failed", false, "SecureShare", share.Id.ToString(), ex.GetBaseException().Message);
        }
    ...
    public async Task<IActionResult> DeleteAfterRetrieve(Guid shareId, string recipientEmail)
    {
        ...
        try
        {
            ...
            await _auditLogger.LogAsync("external-user", normalizedEmail, "share.delete-after-retrieve", true, "SecureShare", shareId.ToString());

            return View("Deleted");
        }
        catch (DatabaseOperationException exception)
        {
            await _auditLogger.LogAsync("external-user", normalizedEmail, "share.delete-after-retrieve", false, "SecureShare", shareId.ToString(), exception.DiagnosticMessage);
    ...
    private async Task<IActionResult?> EnforceShareAccessPauseAsync(SecureShare share, ShareAccessViewModel model, string email)
    {
        ...
        {
            await _auditLogger.LogAsync("external-user", GetAuditIdentifier(email), "share.access", false, "SecureShare", share.Id.ToString(), "Share access paused after failed attempts.");
        ...
        {
            await _auditLogger.LogAsync("external-user", GetAuditIdentifier(email), "share.access", false, "SecureShare", share.Id.ToString(), exception.DiagnosticMessage);
            ModelState.AddModelError(string.Empty, exception.UserMessage);
            return View(model);
        }
    ...
    private async Task<IActionResult> RecordFailedShareAccessAsync(SecureShare share, ShareAccessViewModel model, string email, string details)
    {
        ...
        await _auditLogger.LogAsync("external-user", GetAuditIdentifier(email), "share.access", false, "SecureShare", share.Id.ToString(), auditDetails);
