...
public class AdminController : Controller
{
    ...
    public async Task<IActionResult> Revoke(Guid id)
    {
        ...
        try
        {
            ...
            if (share is null)
            {
                await _auditLogger.LogAsync(actorType, actorIdentifier, "share.revoke", false, "SecureShare", id.ToString(), "Share not found.");
                return NotFound();
            }

            if (!User.IsInRole(_adminRoleName) && !string.Equals(share.CreatedBy, actorIdentifier, StringComparison.OrdinalIgnoreCase))
            {
                await _auditLogger.LogAsync(actorType, actorIdentifier, "share.revoke", false, "SecureShare", id.ToString(), "User attempted to revoke share they do not own.");
                return Forbid();
            }

            await _shareStore.DeleteShareAsync(id);

            await _auditLogger.LogAsync(actorType, actorIdentifier, "share.revoke", true, "SecureShare", id.ToString());
        ...
        {
            await _auditLogger.LogAsync(actorType, actorIdentifier, "share.revoke", false, "SecureShare", id.ToString(), exception.DiagnosticMessage);
