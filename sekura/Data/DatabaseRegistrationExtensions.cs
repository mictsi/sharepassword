...
public static class DatabaseRegistrationExtensions
{
    public static IServiceCollection AddConfiguredStorageBackend(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        switch (backend)
        {
            case StorageOptions.SqliteBackend:
            {
                ...
                services.AddSingleton<ISekuraDbContextFactory, SekuraDbContextFactory<SqliteSekuraDbContext>>();
            ...
            case StorageOptions.SqlServerBackend:
            {
                ...
                services.AddSingleton<ISekuraDbContextFactory, SekuraDbContextFactory<SqlServerSekuraDbContext>>();
                RegisterDatabaseBackedPlatformServices(services);
                break;
            }
            ...
            case StorageOptions.PostgresqlBackend:
            {
                ...
                services.AddSingleton<ISekuraDbContextFactory, SekuraDbContextFactory<PostgresqlSekuraDbContext>>();
                RegisterDatabaseBackedPlatformServices(services);
                break;
            }
    ...
    private static DbContextOptions<SqliteSekuraDbContext> CreateSqliteDbContextOptions(string connectionString)
    {
    ...
    private static DbContextOptions<SqlServerSekuraDbContext> CreateSqlServerDbContextOptions(string connectionString)
    {
        var builder = new DbContextOptionsBuilder<SqlServerSekuraDbContext>();
        builder.UseSqlServer(connectionString, new Action<SqlServerDbContextOptionsBuilder>(ConfigureSqlServerMigrationsAssembly));
        return builder.Options;
    }

    private static DbContextOptions<PostgresqlSekuraDbContext> CreatePostgresqlDbContextOptions(string connectionString)
    {
        var builder = new DbContextOptionsBuilder<PostgresqlSekuraDbContext>();
    ...
    private static void ConfigureSqliteMigrationsAssembly(SqliteDbContextOptionsBuilder sqlite) =>
        sqlite.MigrationsAssembly(typeof(SqliteSekuraDbContext).Assembly.FullName);

    private static void ConfigureSqlServerMigrationsAssembly(SqlServerDbContextOptionsBuilder sqlServer) =>
        sqlServer.MigrationsAssembly(typeof(SqlServerSekuraDbContext).Assembly.FullName);

    private static void ConfigurePostgresqlMigrationsAssembly(NpgsqlDbContextOptionsBuilder npgsql) =>
        npgsql.MigrationsAssembly(typeof(PostgresqlSekuraDbContext).Assembly.FullName);
    ...
    public static async Task ApplyConfiguredStorageMigrationsAsync(this IServiceProvider services, CancellationToken cancellationToken = default)
    {
        ...
        var dbContextFactory = scope.ServiceProvider.GetRequiredService<ISekuraDbContextFactory>();
    ...
    private sealed class StartupDatabaseOperation
    {
        ...
        public StartupDatabaseOperation(ISekuraDbContextFactory dbContextFactory)
        {
