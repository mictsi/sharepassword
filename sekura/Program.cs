...
builder.Services
    ...
    .Validate(
        ...
        "Application:PathBase must be '/' or a relative path like '/sekura'.")
    .Validate(
...
builder.Services
    ...
    .Validate(
        ...
        "Passkey is enabled but Passkey:ServerDomain and/or Passkey:Origins are missing or invalid. ServerDomain must be the site's domain (e.g. \"sekura.example.com\") and Origins must be absolute URLs (e.g. \"https://sekura.example.com\").")
    .ValidateOnStart();
builder.Services.AddSingleton<Fido2NetLib.IFido2>(serviceProvider =>
{
    ...
    return new Fido2NetLib.Fido2(new Fido2NetLib.Fido2Configuration
    {
        ...
        ServerName = string.IsNullOrWhiteSpace(passkeyOptions.ServerName) ? "Sekura" : passkeyOptions.ServerName.Trim(),
